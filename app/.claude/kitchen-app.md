...
- `script.js` — semua logic (state, render, event handler, layer data Firebase). Satu IIFE, vanilla JS, tanpa framework/build tool — pola sama dengan Finance App (lihat catatan soal ES modules vs classic script di `app/.claude/finance-app.md` kalau nanti mau dipecah).
- `app/.claude/kitchen-app.md` — file ini.
...
6. **Pengaturan** (`#settings`)
   ...
   - **Semua Aplikasi** — `.settings-item` biasa di atas "Tentang" berisi tombol `<a class="btn-secondary">Iyon App</a>` (bukan `<button>`, tapi styling sama) mengarah ke `../index.html` (hub Iyon App). CSS tambahan cuma `.settings-item a.btn-secondary` di `css/settings.css` (supaya anchor ter-center & tanpa underline seperti button asli) — pola sama persis ditambahkan juga di Finance App (`finance/css/settings.css` & `finance/index.html`).
   - Info "Tentang".
