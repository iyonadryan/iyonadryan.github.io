...
- **Tab Tulis/Preview** (`#contentEditorTabs`, `.content-editor-tab`) di dalam editor: user awalnya sempat minta transformasi *inline* ala Notion asli (ketik `# ` teksnya langsung berubah besar di tempat yg sama) — **ditolak/disederhanakan** setelah ditawarkan 2 opsi lewat `AskUserQuestion`, krn versi inline butuh ganti textarea jadi contenteditable + parser/serializer 2 arah markdown↔HTML (rawan bug kursor & keyboard mobile/webview). Yang dipilih: **tab terpisah** — "Tulis" nunjukin textarea mentah (default tiap buka editor), "Preview" nunjukin hasil render (`renderMarkdownToHtml`, fungsi yg sama persis dgn popup detail — class `.detail-content` dipakai ulang di `#contentEditorPreview` biar tampilannya konsisten) dari isi textarea **saat itu** (bukan dari `noteContentDraft` yg belum ter-commit, jadi selalu sinkron sama ketikan terakhir). Toggle mode lewat `setContentEditorMode(mode)`, digerakkan oleh class `.preview-mode` di `#contentEditorSheet` (CSS `display:none/block` bergantian antara `.content-editor-textarea` & `.content-editor-preview`). Mode selalu direset ke "Tulis" tiap kali editor dibuka.
...
3. **Popup Detail Catatan** (`#noteDetailModal`)
   ...
   - Tombol **📌 Toggle Sematkan** (icon-btn, langsung tulis Firebase tanpa konfirmasi — low-stakes, gampang di-toggle balik), **Tutup**, **Ubah** → buka modal edit (class `btn-secondary btn-invert` — warna dibalik dari `.btn-secondary` biasa: hitam/teks putih di mode terang, **putih/teks hitam di mode gelap**, reuse `.btn-invert` yg sudah ada di `components.css` (dipakai jg oleh tombol "+ Buat Kategori Baru") spy Ubah lebih menonjol drpd Tutup di sebelahnya — permintaan eksplisit user), **Hapus** → `openConfirm()`.
   - **Dua tombol Tutup** — `#closeNoteDetailBtn` (di `.detail-actions-row`, di antara Pin & Ubah) dan `#detailCloseIconBtn` (ikon ✕ bulat, absolute di pojok kanan-atas card lewat `.detail-close-btn`/`.detail-sheet`, khusus popup ini — **bukan** ditambahkan ke `.modal-sheet` generik spy tidak mempengaruhi modal lain). Dua-duanya + klik area gelap di luar card semua panggil `closeNoteDetail()` yang sama — permintaan eksplisit user spy gampang nutup popup tanpa harus scroll ke bawah cari tombol.
...
5. **Pengaturan** (`#settings`) — Mode Tampilan, **Kategori Catatan — CRUD** (`#categoriesBtn` → `#categoriesModal`, identik pola Kitchen App: list kategori + "+ Buat Kategori Baru", modal Label/Icon/slot-picker 8 warna, delete via `openConfirm()` dgn pesan fallback "Tanpa Kategori"), **Semua Aplikasi** (`<a class="btn-secondary btn-hub">Iyon App</a>` → `../index.html`), Tentang. **Tidak ada row "Pengguna Aktif"** (dihapus, lihat bagian di bawah). Tombol "Iyon App" diberi `.btn-hub` (`css/components.css`) — gradient ungu **sama persis** dgn header hub `app/index.html` (`--gradient-hero` di `app/style-app.css`: light `#6d5bd0`→`#9b6bd6`, dark `#4f3fa6`→`#7a4fb0`, hardcode bukan pakai `--color-primary` app ini sendiri), permintaan eksplisit user spy tombol "pintu balik ke hub" ini konsisten warnanya di semua app — bukan ngikut warna primary masing-masing app. Ditambahkan identik di Kitchen/Finance/Routine/Patungan App.
