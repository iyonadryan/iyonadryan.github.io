...
4. **Pengaturan** (`#settings`) — urutan card: Pengguna Aktif → Mode Tampilan → Semua Aplikasi → Tentang (Pengguna Aktif sengaja paling atas, beda dari Kitchen/Finance yang taruh toggle tema paling atas, krn di app ini siapa yang aktif lebih penting dilihat duluan).
   ...
   - **Semua Aplikasi** — `.settings-item` berisi `<a class="btn-secondary btn-hub">Iyon App</a>` → `../index.html` (hub Iyon App), pola identik dgn Kitchen/Finance App. Tombolnya diberi `.btn-hub` (`css/components.css`) — gradient ungu **sama persis** dgn header hub `app/index.html` (`--gradient-hero` di `app/style-app.css`: light `#6d5bd0`→`#9b6bd6`, dark `#4f3fa6`→`#7a4fb0`, hardcode bukan pakai `--color-primary` app ini sendiri), permintaan eksplisit user spy tombol "pintu balik ke hub" ini konsisten warnanya di semua app — bukan ngikut warna primary masing-masing app. Ditambahkan identik di Kitchen/Finance/Patungan/Note App.
   - Info "Tentang".
...
8. **Multi-user (Iyon / Ciwul / Both)**: app dipakai 2 orang, jadi tiap rutinitas punya field `by` (`"iyon"` | `"ciwul"`) penanda pembuat. **Pola diambil identik dari Finance App** (`finance/script.js`, lihat `app/.claude/finance-app.md` bagian "Multi-user" utk detail lengkap) — bedanya di sini cuma satu jenis data yang di-scope (rutinitas), bukan transaksi+rencana.
   ...
   - **Field "Dibuat oleh" saat TAMBAH rutinitas baru**: modal Tambah Rutinitas (`#routineByField`, toggle `#routineByToggle` — reuse `.mode-toggle`, bukan bikin komponen `.type-toggle` terpisah spt Finance App; label `#routineByLabel`) **selalu tampil** saat create (`setupByFieldForCreate()`), tidak lagi disembunyikan di mode single-user (beda dari desain awal, pola identik Finance App — lihat `app/.claude/finance-app.md`). Kalau `currentUser === "both"`, label tetap "Dibuat oleh" dan toggle aktif/bisa dipilih manual (default "iyon"). Kalau mode aktif Iyon/Ciwul, label berubah jadi **"Dibuat oleh (otomatis)"** dan toggle **dikunci** menyorot user aktif — permintaan eksplisit user spy pembuatnya tetap kelihatan jelas, bukan diam-diam. `routineFormBy()` saat submit tetap ambil `currentUser` langsung di mode single-user (toggle terkunci cuma visual).
   - **Field "Dibuat oleh" saat EDIT**: perilaku lama dipertahankan — `setupByFieldForEdit(by)` menyembunyikan field **kecuali** `currentUser === "both"`, toggle **selalu dikunci** ke `routine.by` (`setByToggleValue(..., true)`) — pembuat tidak bisa diubah retroaktif. Kedua fungsi menggantikan `updateByFieldVisibility()` lama (dihapus) — state di-set eksplisit tiap `openRoutineModal(routine)` dipanggil (`routine` null = create, terisi = edit), bukan lewat listener terpisah saat ganti user.
