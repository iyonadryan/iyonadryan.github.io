...
2. **Wishlist** (`#wishlist`) — daftar & CRUD item **aktif** (belum tercapai — item tercapai pindah ke halaman Tercapai, lihat poin 4).
   ...
   - **Ringkasan Total** (`.wishlist-total-row`, di antara tab filter Prioritas & list item): label "TOTAL" + `formatCurrency()` dari **penjumlahan `price` semua item yg lolos filter aktif** (kategori + prioritas + search sekaligus, sama tiga kondisi persis yg dipakai `renderWishlistList()` mem-filter `filtered[]` sblm dirender jadi kartu — `document.getElementById("wishlistTotalValue").textContent` di-set dari `filtered.reduce((sum,i)=>sum+(Number(i.price)||0),0)`, dihitung **ulang tiap kali `renderWishlistList()` jalan** jadi otomatis ikut berubah tiap filter/search diganti, tanpa listener terpisah). Cuma menjumlah item **aktif** (blm tercapai) — item di halaman Tercapai tidak py ringkasan Total serupa.
