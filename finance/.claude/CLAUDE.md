...
4. **Pengaturan** (`#settings`)
   ...
   - **Semua Aplikasi** — `.settings-item` biasa di atas "Tentang" berisi tombol `<a class="btn-secondary">Iyon App</a>` (bukan `<button>`, tapi styling sama) mengarah ke `../app.html` (hub Iyon App). CSS tambahan cuma `.settings-item a.btn-secondary` di `css/settings.css` (supaya anchor ter-center & tanpa underline seperti button asli) — pola sama persis ditambahkan juga di Kitchen App (`kitchen/css/settings.css` & `kitchen/index.html`).
