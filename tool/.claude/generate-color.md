...
**Bug: default warna dasar kadang bukan `#0F6E63` pas buka/refresh halaman** — akar masalahnya browser Chromium (& turunannya) me-restore value terakhir sebuah `<input>` teks/angka pas halaman di-reload biasa (Ctrl+R/F5), **walau elemennya tidak di dalam `<form>`** dan walau `value="..."` di HTML-nya tetap `#0F6E63`. Ini bagian dari state riwayat navigasi (session history) milik browser, **bukan** fitur autofill biasa. Diperbaiki bertahap (perbaikan pertama ternyata belum cukup, lihat catatan "belum cukup" di bawah):

1. `DEFAULT_BASE_HEX` konstanta terpisah — `currentBaseHex` tidak pernah lagi dibaca dari `baseColorPicker.value`/DOM saat init.
2. `INIT` manggil `setBaseColor(DEFAULT_BASE_HEX)` scr eksplisit di awal, maksa SEMUA field (picker, hex, R, G, B) balik ke default kanonik.
3. `autocomplete="off"` ditambahkan ke kelima input sbg lapisan tambahan.
4. **(Belum cukup sampai di sini)** — restorasi value oleh Chromium ternyata bisa terjadi **SETELAH** script ini selesai jalan (bukan cuma sebelumnya), krn ini bagian dari commit navigasi/session-history, bukan cuma parsing HTML awal — jadi fix #1-3 di atas kadang masih ke-timpa lagi belakangan. Solusi final: listener `window.addEventListener('pageshow', () => setBaseColor(DEFAULT_BASE_HEX))` — event `pageshow` dijamin nyala SETELAH seluruh proses restorasi browser (baik reload normal maupun restore dari bfcache back/forward) selesai, jadi manggil `setBaseColor(DEFAULT_BASE_HEX)` lagi di situ yang benar2 "menang belakangan". Konsekuensinya `setBaseColor` kepanggil 2× tiap load normal (sekali di INIT, sekali lagi di `pageshow`) — idempoten & murah, sengaja dibiarkan drpd nambah kerumitan skip-logic cuma demi menghindari 1 render ekstra.
