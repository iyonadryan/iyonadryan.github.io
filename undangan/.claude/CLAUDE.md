...
23. Foto `.couple-photo` DIGANTI LAGI — user nambah folder baru `photo2/` (`groom.jpeg`/`bride.jpeg`, foto bust portrait studio SUDAH square 2731×2731) & minta pakai foto ini, TAPI eksplisit bilang **"gak perlu di-crop"** (beda dari `photo/` sblmnya yg emang di-crop wajah manual) — jadi cuma di-resize+convert WebP APA ADANYA (utuh, termasuk ruang kosong abu2 di atas kepala krn orangnya cuma isi ±2/3 bawah frame), TANPA crop wajah sama sekali. Lihat "Foto Profil Mempelai" utk detail & perbandingan dgn pendekatan `photo/` yg lama.
24. Section Galeri sempat DICOBA diganti dari grid 3 kolom statis jadi **carousel geser** (native CSS scroll-snap + dot indicator, permintaan eksplisit user "coba implementasikan dan saya pingin lihat hasilnya") — TAPI stlh dilihat hasilnya, user minta **ROLLBACK ke grid 3 kolom semula**. Keinginan "mode geser" user sbenernya lbh ke arah **lightbox popup-nya** (bukan grid galeri) — jadi ditambahkan di situ sbg gantinya: tombol next/prev + swipe gesture DI DALAM popup lightbox, dgn navigasi **looping** (foto ke-6 → next → balik ke foto ke-1, & sebaliknya). Lihat "Navigasi Lightbox (Next/Prev + Swipe, Looping)".
25. Jam selesai Resepsi DIREVISI dari **14.00 WIB → 16.00 WIB** (permintaan eksplisit user, jam mulai 11.00 & jam Akad TIDAK berubah) — update di `event-datetime` (index.html) & `end` link "Tambah ke Kalender" (script.js, `070000Z`→`090000Z`) — lihat "Data acara".
26. Tanggal acara sudah lewat, user khawatir **Firebase bisa berubah/mati 10 tahun lagi** & pingin nyimpen "hasil index yang sekarang" sbg kenang-kenangan **tanpa bergantung ke Firebase sama sekali** — ditambahkan **`index-history.html`** (+ `script-history.js`) sbg arsip statis independen, lihat "Arsip Kenangan (`index-history.html`)".
...
undangan/
  ...
  style.css        # semua styling index.html (token warna di :root, animasi, responsive) — DIPAKAI
                     # BARENG jg oleh index-history.html, lihat "Arsip Kenangan"
  script.js        # logic index.html (vanilla JS, tanpa framework)
  index-history.html # ARSIP statis kenang-kenangan, TIDAK terhubung Firebase sama sekali — lihat
                     # "Arsip Kenangan (`index-history.html`)"
  script-history.js  # logic index-history.html — turunan script.js TANPA bagian yg butuh Firebase
                     # (RSVP live listener/submit, countdown ticking dihapus jg)
...
- **Resepsi**: 11.00 — 16.00 WIB (DIREVISI dari 11.00–14.00 semula, permintaan eksplisit user susulan — cuma jam selesai yg mundur, jam mulai & Akad TIDAK berubah), **Masjid Raya Al Ikhlas** — **SAMA PERSIS lokasinya dgn Akad** (permintaan eksplisit user, beda dari placeholder lama yg py 2 venue beda — Kediaman Mempelai Wanita utk Akad, Graha Kirana Ballroom utk Resepsi).
...
- Tombol "+ Tambah ke Kalender" generate link Google Calendar (`calendar.google.com/calendar/render?action=TEMPLATE`), rentang waktu event **UTC** `20260802T020000Z` — `20260802T090000Z` (= 09.00–16.00 WIB, mencakup Akad s/d selesai Resepsi; WIB = UTC+7, jadi kurangi 7 jam dari waktu lokal saat ubah ke Z — `end` DIREVISI dari `070000Z` ke `090000Z` seiring jam selesai Resepsi mundur ke 16.00). **Kalau tanggal/jam acara berubah lagi, WAJIB update 3 tempat sekaligus**: `cover-date` + `event-datetime` (index.html, ada 2 — Akad & Resepsi), `target` countdown, dan `start`/`end` calendar (script.js) — tidak otomatis saling sinkron, semua hardcoded literal.
...
- 6 foto asli, ditampilkan **grid 3 kolom** (`.gallery-grid`, `aspect-ratio:1/1`, `object-fit:cover`) — `<img src="gallery2/*.webp">` di `index.html` (lihat "Optimasi Galeri (WebP)" di bawah utk kenapa `.webp`, bukan `.jpg`/`.jpeg` lagi — DAN kenapa `gallery2/`, bukan `gallery/`). **Sempat dicoba diganti carousel geser (scroll-snap), lalu di-ROLLBACK ke grid ini lagi** atas permintaan user stlh lihat hasilnya — lihat "Status saat ini" poin 24 & "Navigasi Lightbox" di bawah utk ke mana perginya fitur "geser"-nya.
- Tiap foto dibungkus `<button class="gallery-item">` (bukan `<div>`) supaya bisa diklik & accessible (fokus keyboard). Klik → `#lightbox` overlay fullscreen (`script.js`, IIFE "GALLERY LIGHTBOX"): set `src`/`alt` dari foto yg diklik, toggle class `.open`. Tutup lewat tombol ✕, klik area gelap di luar gambar, atau tombol **Esc** — DAN SEKARANG BISA PINDAH FOTO tanpa nutup dulu, lihat "Navigasi Lightbox" di bawah.
- Foto profil bulat mempelai (`.couple-photo`, section "Kedua Mempelai") **beda dari galeri** — bukan bagian dari galeri/lightbox, foto individual sendiri, lihat "Foto Profil Mempelai" di bawah.

### Navigasi Lightbox (Next/Prev + Swipe, Looping)

Permintaan eksplisit user, muncul stlh eksperimen "Carousel Galeri" (lihat "Status saat ini" poin 24) di-rollback — ternyata yg user maksud "mode digeser" itu bukan galeri grid-nya yg diubah, tapi **popup lightbox (tampilan foto ukuran penuh)** yg perlu bisa digeser/next ke foto lain TANPA perlu nutup popup dulu balik ke grid — plus tombol next eksplisit, DAN **looping** (foto ke-6 → next → balik ke foto ke-1, foto ke-1 → prev → balik ke foto ke-6).

- **`index.html`**: `#lightbox` skrg py 2 tombol baru — `#lightboxPrev` (`&#10094;` ❮) di kiri, `#lightboxNext` (`&#10095;` ❯) di kanan, di kanan-kiri `<img id="lightboxImg">`, sejajar `#lightboxClose` yg udah ada.
- **`script.js`, IIFE "GALLERY LIGHTBOX" ditulis ulang** — sebelumnya `openLightbox(src, alt)` nerima src/alt langsung dari elemen yg diklik (gak nyimpen "lagi di foto keberapa"), SEKARANG jadi berbasis **index**: `items` = array semua `.gallery-item` (urutan DOM = urutan foto 1-6), `currentIndex` state tersimpan di closure, `showIndex(i)` yg update `currentIndex` & isi `lightboxImg`.
  - **Looping via modulo**: `showIndex(index)` ngitung `currentIndex = (index + items.length) % items.length` — trik `+ items.length` sblm modulo krn JS `%` bisa hasil negatif kalau `index` negatif (mis. index -1 di array 6 elemen, `-1 % 6` di JS = `-1`, BUKAN `5` spt di Python) — nambahin `items.length` dulu (`-1 + 6 = 5`) baru di-modulo jamin hasil selalu positif & valid buat index array, ini yg bikin next dari foto terakhir "wrap" ke foto pertama & sebaliknya, OTOMATIS tanpa perlu `if (index >= items.length) index = 0` manual.
  - `showNext()`/`showPrev()` cuma manggil `showIndex(currentIndex +/- 1)` — logic wrap-nya udah ditangani `showIndex` sendiri, gak perlu duplikasi cek batas di 2 fungsi ini.
- **3 cara trigger next/prev**, semuanya manggil fungsi generik yg sama (`showNext`/`showPrev`), gak ada logic ke-duplikasi antar metode:
  1. **Klik tombol** `#lightboxNext`/`#lightboxPrev`.
  2. **Tombol panah keyboard** (`ArrowRight`/`ArrowLeft`) — cuma aktif kalau lightbox lagi `.open` (dicek di awal listener, biar gak nyangkut ke navigasi lain di halaman kalau lightbox lagi ketutup).
  3. **Swipe/geser** (touch) — `touchstart` simpen `clientX` awal, `touchend` hitung `deltaX` = posisi akhir - awal, geser ke KIRI (`deltaX <= -40`) → `showNext()`, geser ke KANAN (`deltaX >= 40`) → `showPrev()`. Threshold `40px` nyegah tap tipis/gak sengaja kebaca sbg swipe. Listener dipasang `{ passive:true }` (gak pernah manggil `preventDefault()`) krn lightbox emang gak py scroll internal yg perlu dicegah — lebih ringan/gak ganggu scrolling browser bawaan.
- **Bug ditemukan & diperbaiki sekalian (real bug, bukan cuma teori): tombol `#lightboxPrev` KETUTUPAN `<img>`, gak keklik sama sekali** — `.lightbox img` py CSS `transform: scale(...)` (efek zoom-in animasi buka), dan `transform` APA PUN (walau `scale(1)`, identity transform sekalipun) **otomatis bikin elemen itu jadi stacking context baru** setara `position:relative;z-index:0` scr implisit (spesifikasi CSS) — akibatnya `<img>` (yg letaknya di ANTARA `#lightboxPrev` dan `#lightboxNext` di HTML) ikut "bersaing" di level stacking yg sama dgn 2 tombol nav itu (`.lightbox-nav`, position:absolute, tanpa z-index eksplisit), dan krn urutan DOM `prev → img → next`, `img` menang stacking di ATAS `prev` (tapi KALAH drpd `next` yg dateng stlh dia) — hasilnya tombol PREV doang yg ketutup gak bisa diklik, tombol NEXT normal (kebetulan konsekuensi urutan HTML). **Fix**: tambah `z-index:2` eksplisit ke `.lightbox-close` & `.lightbox-nav` — angka pasti menang drpd stacking-context implisit `transform` yg auto ~`z-index:0`, gak peduli urutan DOM lagi. **Pelajaran sama kayak bug `isolation` di "Unsur Adat" Tahap 3**: elemen ber-`transform`/`opacity`/dll yg bikin stacking context implisit itu jebakan berulang di repo ini — kalau nambah tombol/elemen overlay `position:absolute` baru di atas elemen lain yg py `transform`, JANGAN asumsikan urutan DOM/posisi otomatis benar, kasih `z-index` eksplisit drpd ketauan pas testing/dipakai user.
- **Grid galeri (`.gallery-item`) & scroll-reveal SAMA SEKALI TIDAK PERLU disentuh** utk fitur ini — `items` di JS baru dibaca dari `.gallery-item` yg sama persis, cuma cara `openLightbox` dipicu yg berubah (skrg kirim index posisi, bukan src/alt langsung).
- **Diuji eksplisit** (Playwright + Edge headless, viewport 390×844, `hasTouch:true`): buka foto ke-1 → klik next 5x → TERBUKTI nyampe foto ke-6; klik next 1x lagi dari foto ke-6 → TERBUKTI **loop balik ke foto ke-1**; klik prev dari foto ke-1 → TERBUKTI **loop ke foto ke-6** (looping 2 arah kebukti jalan); `ArrowLeft` keyboard TERBUKTI mundur 1 foto; simulasi touch swipe (dispatch `TouchEvent` manual, geser kiri) TERBUKTI maju ke foto selanjutnya — semua tanpa page error.
...
- **Alamat SUDAH DIGANTI LAGI** (riwayat lengkap, biar gak bingung baca versi mana yg berlaku SEKARANG):
  1. `KP TANAH 80 NO.2 RT.010/RW.008 KLENDER DUREN SAWIT JAKARTA TIMUR DKI JAKARTA` (versi awal, all-caps, dari input asli user).
  2. Sempat diganti total ke `Kp. Tanah 80 No. 2 RT 010 RW 08 Klender Jakarta Timur 13470` (kapitalisasi normal, tanpa nama kecamatan, dari screenshot user).
  3. Di-ROLLBACK ke versi #1 + tambahan `13470` di akhir → `KP TANAH 80 NO.2 RT.010/RW.008 KLENDER DUREN SAWIT JAKARTA TIMUR DKI JAKARTA 13470`.
  4. **VERSI SEKARANG (final, permintaan eksplisit user)**: **`Jl. Madrasah No.2 B, RT.10/RW.8, Klender, Kec. Duren Sawit, Kota Jakarta Timur, Daerah Khusus Ibukota Jakarta 13470`** — alamat baru sepenuhnya (nama jalan beda — "Jl. Madrasah" gantiin "Kp. Tanah"/"KP TANAH" — bukan cuma reformat versi lama), kapitalisasi normal, pakai koma sbg pemisah komponen alamat, kecamatan & "Kota Jakarta Timur"/"Daerah Khusus Ibukota Jakarta" disebut lengkap, kode pos `13470` di akhir.
  - Ditulis **PERSIS apa adanya** sesuai input user tiap kali diganti (prinsip "display = copy value, apa adanya" — TIDAK pernah diformat ulang manual oleh Claude, kapitalisasi/tanda baca/susunan komponen alamat ikut apa yg dikasih user setiap revisi).
  - **Diuji eksplisit tiap revisi** (Playwright + Edge headless + `grantPermissions(['clipboard-read','clipboard-write'])`): teks tampil (`.gift-address-text`), `data-copy`, DAN hasil `navigator.clipboard.readText()` stlh klik "Salin Alamat" TERBUKTI 3-3-nya identik persis dgn versi alamat yg lagi aktif.
...
- **Tombol "Lihat Lokasi" ditambahkan belakangan** (permintaan eksplisit user susulan, `https://maps.app.goo.gl/MdepXhS3t4iBDki19` — link share Maps asli dari user, sama pola/prinsipnya dgn "Lihat Lokasi" di kartu Akad/Resepsi, lihat "Data acara": dipakai apa adanya, `target="_blank" rel="noopener noreferrer"`, KALAU lokasi ganti lagi nanti WAJIB minta link share Maps baru ke user, bukan dikonstruksi manual dari teks alamat) — SEBELUMNYA kartu ini cuma py 1 tombol ("Salin Alamat"), sekarang 2 tombol berdampingan.
  - **Class baru `.gift-actions`** (`style.css`: `display:flex; justify-content:center; flex-wrap:wrap; gap:10px;`) MEMBUNGKUS kedua tombol (`<button class="copy-btn">` + `<a class="btn-outline">`) — dibutuhkan krn sblmnya cuma 1 tombol/kartu di mana pun jadi belum pernah ada kebutuhan atur jarak antar-2-tombol sejajar; `flex-wrap:wrap` jaga2 di layar sangat sempit 2 tombol itu turun ke baris baru drpd numpuk/kepotong.
  - **Kartu bank (BNI/BCA) TIDAK ikut dibungkus `.gift-actions`** — keduanya tetap cuma 1 tombol ("Salin Nomor"), jadi tidak butuh wrapper flex ini, cuma kartu "Kirim Kado Fisik" yg pakai.
  - **Diuji eksplisit** (Playwright + Edge headless): `href` tombol TERBUKTI persis link share Maps yg dikasih user, `target="_blank"` & `rel="noopener noreferrer"` TERBUKTI ada, screenshot TERBUKTI kedua tombol ("Salin Alamat"/"Lihat Lokasi") tampil rapi berdampingan di bawah teks alamat.
...
Permintaan eksplisit user — tanggal acara (2 Agustus 2026) udah lewat, user khawatir **10 tahun lagi Firebase project ini bisa berubah/dimatikan/di-migrate**, jadi mau nyimpen "hasil index yang sekarang" (snapshot tampilan situs persis spt kondisi sekarang) sbg kenang-kenangan yg **TIDAK bergantung ke Firebase sama sekali** — kalau project Firebase-nya beneran mati suatu hari, halaman arsip ini TETAP jalan normal (semua data yg butuh Firebase sudah dibekukan jadi teks statis di HTML-nya sendiri).

- **2 file baru, TERPISAH dari `index.html`/`script.js`** (bukan modifikasi in-place — `index.html` yg asli TETAP hidup normal terhubung Firebase spt biasa, arsip ini cuma DUPLIKAT beku): **`index-history.html`** + **`script-history.js`**. `style.css` DIPAKAI BARENG (`<link rel="stylesheet" href="style.css">` sama persis) — styling visual harus identik dgn versi live, jadi tidak masuk akal duplikat file CSS terpisah cuma utk halaman yg strukturnya 99% sama.
- **`<head>` `index-history.html` SAMA SEKALI TIDAK PUNYA blok Firebase** — 2 `<script src="...firebasejs...">` CDN + `firebaseConfig`/`firebase.initializeApp()`/`const db` (yg ada di `<head>` `index.html`) **DIHAPUS TOTAL**, bukan cuma dikomentari — jadi halaman ini beneran nol ketergantungan ke domain/service Firebase apa pun, bukan cuma "gak dipakai tapi tetep connect diam2".
- **`script-history.js` = turunan `script.js`** (bukan file baru dari nol) — SEMUA fitur yg TIDAK butuh Firebase disalin apa adanya (staggered reveal, kelopak bunga jatuh, buka undangan, scroll reveal, toggle musik, toast, copy alamat/rekening, lightbox galeri dgn navigasi next/prev/swipe) — yg DIHAPUS cuma 2 blok:
  1. **IIFE "RSVP & UCAPAN (Firebase)"** (`rsvpRef`, `renderWishList()`, listener `rsvpRef.on('value', ...)`, handler submit `#rsvpForm`) — dihapus total, krn `index-history.html` toh gak py elemen `#rsvpForm` sama sekali (lihat di bawah) & isi `#wishList` sudah statis langsung di HTML, gak perlu di-render ulang oleh JS.
  2. **IIFE "COUNTDOWN"** (`setInterval` tiap detik ngitung mundur ke `target`) — dihapus, gantiin liat "Countdown versi arsip" di bawah.
- **Section "RSVP & Ucapan" DIBEKUKAN jadi "Ucapan & Doa" statis** (permintaan eksplisit user, opsi yg dipilih dari 2 alternatif yg ditawarkan — bukan dihapus totalnya section): `<form id="rsvpForm">` **DIHAPUS TOTAL** (submit gak mungkin jalan tanpa Firebase, nyisain form yg gak berfungsi cuma bikin bingung), `#wishList` diisi **HARDCODE HTML statis** — snapshot SEMUA ucapan yg beneran ada di Firebase (`undangan/rsvp`) **PAS TANGGAL ARSIP INI DIBUAT** (dibaca sekali via `curl` REST publik, BUKAN via SDK/koneksi browser — TIDAK ada permintaan Firebase apa pun terjadi dari `index-history.html` itu sendiri, cuma dari sesi kerja Claude pas nyusun arsipnya), **13 ucapan**, diurutkan sama persis logic `renderWishList()` asli (terbaru dulu, by `createdAt` descending), markup (`.wish-card`/`.wish-card--hadir`/`.wish-name`/`.wish-text`) & escaping HTML (`&`/`<`/`>`/dst → entity, newline pesan → `<br>`) DIBUAT SAMA PERSIS kayak hasil `renderWishList()` JS asli — bedanya cuma sekarang ditulis statis di HTML, bukan digenerate JS saat load.
  - **1 ucapan (nama "Subcribe", isi "bit.ly/prakoop2") KELIHATAN kayak spam/promosi** (bukan ucapan pernikahan wajar) — TETAP DIMASUKKAN APA ADANYA ke snapshot (prinsip "arsip = salinan JUJUR dari data yg beneran ada", bukan disaring/disensor sepihak oleh Claude) TAPI SUDAH DIFLAG eksplisit ke user pas arsip ini dibikin — kalau mau dihapus dari arsip, user perlu edit manual `index-history.html` sendiri (hapus 1 blok `<div class="wish-card">` yg namanya "Subcribe").
  - Section-eyebrow diganti dari "Konfirmasi Kehadiran" → **"Kenangan"**, judul dari "RSVP & Ucapan" → **"Ucapan & Doa"** (krn "konfirmasi kehadiran" gak relevan lagi utk acara yg udah lewat, cuma daftar ucapannya yg masih relevan disimpan).
- **Section "Countdown" diganti jadi teks statis** (permintaan eksplisit user, opsi yg dipilih dari 2 alternatif) — 4 kotak angka berjalan (`#cdDays`/`#cdHours`/`#cdMinutes`/`#cdSeconds`) + tombol "+ Tambah ke Kalender" **DIHAPUS TOTAL** (nambah kalender utk acara yg udah lewat gak relevan), diganti 2 baris teks statis: `<p class="countdown-static">Minggu, 2 Agustus 2026</p>` + `<p class="countdown-static-sub">Telah menjadi hari pernikahan kami</p>`. Section-eyebrow "Save The Date" → **"Kenangan"**, judul "Menuju Hari Bahagia" → **"Hari Bahagia Kami"** (present/past framing, bukan future).
  - **2 class CSS baru** (`.countdown-static`/`.countdown-static-sub`) ditambahkan ke `style.css` YG DIPAKAI BARENG — AMAN utk `index.html` yg live krn cuma NAMBAH selector baru yg gak dipakai di situ, TIDAK mengubah/menimpa selector yg sudah ada (`.countdown`/`.cd-box`/`.cd-num` dkk yg dipakai `index.html` tetap utuh apa adanya).
- **Section lain (Cover, Kutipan, Mempelai, Rangkaian Acara, Galeri, Amplop Digital, Penutup) DISALIN APA ADANYA** — tidak ada satu pun yg butuh Firebase, jadi tidak ada alasan diubah dari versi live. Termasuk `?to=NamaTamu` (personalisasi sapaan cover) TETAP dipertahankan di `script-history.js` — walau kegunaannya utk bagi-bagi link personal per-tamu udah gak relevan lagi (acara udah lewat), fiturnya sendiri harmless & murni baca query string (nol Firebase), jadi dibiarkan drpd dihapus tanpa alasan kuat.
- **Kalau `index.html` (versi live) berubah lagi ke depan** (foto ganti, alamat ganti, dst) — `index-history.html` **TIDAK otomatis ikut berubah** (memang sengaja beku, itu intinya "arsip") — kalau user MAU update arsipnya jg (bukan cuma index.html live), harus diminta terpisah & eksplisit, jangan diasumsikan otomatis tiap kali edit `index.html`.
- **Diuji eksplisit** (Playwright + Edge headless, network request di-`page.on('request', ...)`-log): TERBUKTI **nol request** ke domain apa pun yg mengandung "firebase" (baik SDK CDN `gstatic.com/firebasejs` maupun `*.firebaseio.com`); TERBUKTI global `window.firebase`/`db` **tidak ada** sama sekali di `window` (bukan cuma "gak dipanggil", beneran gak pernah didefinisikan); countdown TERBUKTI teks statis "Minggu, 2 Agustus 2026" (bukan lagi `#cdDays` dkk); `#rsvpForm` TERBUKTI tidak ada di DOM; `#wishList` TERBUKTI berisi 13 `.wish-card`, urutan terbaru dulu ("Life Seeker Group" nongol pertama); tombol salin alamat/nomor rekening TERBUKTI tetap berfungsi (clipboard sungguhan, `navigator.clipboard.readText()` cocok); lightbox galeri TERBUKTI tetap buka normal.
