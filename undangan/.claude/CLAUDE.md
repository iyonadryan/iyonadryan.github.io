...
- **Alamat**: `KP TANAH 80 NO.2 RT.010/RW.008 KLENDER DUREN SAWIT JAKARTA TIMUR DKI JAKARTA 13470` — ditulis **PERSIS apa adanya** (all-caps, singkatan RT/RW dgn titik) sesuai input asli user, TIDAK diformat ulang (kapitalisasi/tanda baca) krn resiko salah transkripsi/tafsir struktur alamat (RT/RW mana, kelurahan/kecamatan mana) yg tidak eksplisit dijelaskan user — konsisten dgn prinsip "display = copy value, apa adanya" yg baru ditegaskan user di kartu bank (lihat di atas). **Sempat diganti total ke versi kapitalisasi normal + tanpa nama kecamatan** (permintaan user, dari screenshot), TAPI lalu **DIROLLBACK ke versi all-caps semula** (permintaan susulan "balikin ke yang awal") — **kode pos `13470` dari versi sisipan itu TETAP DIPERTAHANKAN**, ditambahkan di akhir versi all-caps asli (user eksplisit minta "tambahin 13470 belakangnya" pas rollback, jadi bukan rollback murni 100% ke teks lama, tapi versi lama + tambahan kode pos).
