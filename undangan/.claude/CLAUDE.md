...
- **`.music-icon-play` (`▶︎`) & `.music-icon-pause` (`❚❚`)** — 2 `<span aria-hidden="true">` ditumpuk di dalam `#musicToggle` (`index.html`), CUMA 1 yg kelihatan tergantung status (`.music-toggle.playing .music-icon-play{display:none}` / `...music-icon-pause{display:block}`, kebalikannya default) — gantiin `<svg class="disc">...</svg>` lama sepenuhnya (`class="disc"`/`class="disc-dot"` SUDAH TIDAK ADA lagi di kode manapun).
...
- **Rule `prefers-reduced-motion`** (`@media (prefers-reduced-motion: reduce)`, di `style.css`) diperbarui: `.disc` di daftar selector yg dimatikan animasinya diganti jd `.music-icon` — kalau tidak, aturan aksesibilitas ini diam-diam jadi tidak berlaku lagi krn `.disc` sudah tidak ada elemen manapun yg pakai.
- **Diuji eksplisit** (Playwright + Edge headless): state awal (blm diklik) → `▶︎` TERBUKTI kelihatan, `❚❚` TERBUKTI tersembunyi, `animation-play-state:paused`; klik tombol → `.playing` class TERBUKTI nempel, `▶︎`↔`❚❚` TERBUKTI tertukar visibility-nya, `animation-play-state:running`, `bgm.paused` TERBUKTI `false` (musik beneran jalan, bukan cuma ikon doang yg berubah); klik lagi → semua TERBALIK lagi (`bgm.paused:true`) — dikonfirmasi jg via screenshot kedua ikon scr visual jelas beda bentuk (segitiga solid vs 2 batang solid).
